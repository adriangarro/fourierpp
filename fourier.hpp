 ...
 * Utility: Implements Discrete Fourier Transform Algorithms
 * Built with ISO C++14
 */
...
namespace fourierpp {
    ...
    void adjust_zero_precision(std::complex<double>& point)  // close to zero? you're zero
    {
    ...
    template <class signal_vector>  // points of signal close to zero become zero
    ...
    template <class signal_vector>
    signal_vector sdft(signal_vector& data) // Simple DFT (not in-place)
    {
        auto data_size = data.size();
        signal_vector result(data_size);
        for (auto freq = 0; freq < data_size; ++freq) {  // for every frequency...
            for (auto t = 0; t < data_size; ++t) {  // for every point in time...
                auto twiddle_factor = std::polar(1.0, -2 * pi * freq * t / data_size);
                auto result_point = data[t] * twiddle_factor;  // datapoint * e^(-i*2*pi*f)
                               ...
                               signal_vector& odd, signal_vector& even) 
    {
        for (auto index = 0; index < data_size; ++index) {
            if ((index % 2) != 0) {
                odd.push_back(data.at(index));
            } else {
                even.push_back(data.at(index));
            }
        }
    }
    
    template <class signal_vector>
    void aux_fft(signal_vector& data)  // Cooley–Tukey FFT (in-place)
    {
        auto data_size = data.size();
    ...
    template <class signal_vector>
    signal_vector fft(signal_vector& data)  // Cooley–Tukey FFT (not in-place)
    {
    ...
    template <class signal_vector>
    signal_vector idft(signal_vector& data)  // Inverse DFT (not in-place)
    {
        auto data_size = data.size();
        signal_vector result(data_size);
